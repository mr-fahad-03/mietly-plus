...
export default function AdminEditProductPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchAdminCategoryTree(token), fetchAdminBrands(token), fetchAdminProducts(token)])
      .then(([cats, brandList, products]) => {
        setCategories(cats);
        setBrands(brandList);
        ...
        setForm({
          titleEn: product.titleI18n?.en || product.title || "",
          titleDe: product.titleI18n?.de || "",
          slug: product.slug,
          sku: product.sku || "",
          brandId: product.brandId || "",
          shortDescriptionEn: product.shortDescriptionI18n?.en || product.shortDescription || "",
          shortDescriptionDe: product.shortDescriptionI18n?.de || "",
          descriptionEn: product.descriptionI18n?.en || product.description || "<p></p>",
          descriptionDe: product.descriptionI18n?.de || "<p></p>",
          imageUrl: product.imageUrl || "",
          galleryImagesText: "", // Handled visually now
          ...
          maxRentalQuantity: String(product.maxRentalQuantity ?? 1),
          minimumRentalWeeks: String(product.minimumRentalWeeks ?? convertDaysToWeeks(product.minimumRentalDays || 7)),
          maximumRentalWeeks: String(product.maximumRentalWeeks ?? convertDaysToWeeks(product.maximumRentalDays || 30)),
          minimumRentalMonths: String(product.minimumRentalMonths ?? 1),
          maximumRentalMonths: String(product.maximumRentalMonths ?? 24),
          ...
          replacementValue: String(product.replacementValue ?? 0),
          unit: product.unit || "piece",
          weightKg: String(product.weightKg ?? 0),
          tagsText: (product.tags || []).join(", "),
          specificationsText: (product.specifications || []).map((item) => `${item.key}: ${item.value}`).join("\n"),
          seoMetaTitle: product.seo?.metaTitle || "",
          seoMetaDescription: product.seo?.metaDescription || "",
          seoMetaKeywords: (product.seo?.metaKeywords || []).join(", "),
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!adminToken || !params?.id) return;
    ...
    setError("");
    try {
      await updateProduct(adminToken, params.id, {
      ...
      setMessage("Rental product updated.");
    } catch (err) {
  ...
  if (loadingProduct) return <p className="text-sm text-zinc-500">Loading product...</p>;

  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <p className="text-sm text-zinc-600">Edit rental product with bilingual content and rental settings.</p>
        <button type="button" onClick={() => router.push("/admin/products/list")} className="rounded-lg border border-zinc-300 px-3 py-1.5 text-sm text-zinc-700">
          Back to Product List
        </button>
      </div>

      <section className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
        <h1 className="text-2xl font-bold text-zinc-900">Edit Rental Product</h1>
        <form className="mt-4 space-y-6" onSubmit={onSubmit}>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div><span className="mb-1 block text-sm">Long Description (EN) - Tiptap</span><RichTextEditor value={form.descriptionEn} onChange={(value)=>setForm((p)=>({...p,descriptionEn:value}))} adminToken={adminToken}/></div>
            <div><span className="mb-1 block text-sm">Long Description (DE) - Tiptap</span><RichTextEditor value={form.descriptionDe} onChange={(value)=>setForm((p)=>({...p,descriptionDe:value}))} adminToken={adminToken}/></div>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            <div>
            ...
            <div className="rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2 text-sm text-zinc-600">
              Images are managed from upload. {uploadedImages.length > 0 ? `${uploadedImages.length} image(s) set.` : "No image yet."}
            </div>
          </div>
          ...
          <div className="rounded-xl border border-zinc-200 p-4">
            ...
            <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.depositEnabled} onChange={(e)=>setForm((p)=>({...p,depositEnabled:e.target.checked}))}/>Enable deposit for this product</label>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <label className="block"><span className="mb-1 block text-sm">Tags</span><input value={form.tagsText} onChange={(e)=>setForm((p)=>({...p,tagsText:e.target.value}))} className="w-full rounded-lg border border-zinc-300 px-3 py-2"/></label>
          ...
          <div className="rounded-xl border border-lime-200 bg-lime-50/40 p-4">
            <p className="mb-3 font-semibold">SEO Settings</p>
          ...
          <button type="submit" disabled={loading || uploading} className="rounded-xl bg-lime-500 px-5 py-2 font-semibold text-white disabled:opacity-60">
            {loading ? "Updating..." : "Update Rental Product"}
          </button>
        </form>
